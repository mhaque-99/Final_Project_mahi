...
Gender distribution was similar across groups. Among Non-URM students, `r round(gender_tab$pct[gender_tab$URM_bin == "Non-URM" & gender_tab$gender_clean == "F"][1], 1)`% were female and `r round(gender_tab$pct[gender_tab$URM_bin == "Non-URM" & gender_tab$gender_clean == "M"][1], 1)`% were male. Among URM students, `r round(gender_tab$pct[gender_tab$URM_bin == "URM" & gender_tab$gender_clean == "F"][1], 1)`% were female and `r round(gender_tab$pct[gender_tab$URM_bin == "URM" & gender_tab$gender_clean == "M"][1], 1)`% were male.

Race distributions differed substantially between URM and non-URM students. Among non-URM students, the majority identified as White (`r race_tab$pct[race_tab$URM_bin=="Non-URM" & race_tab$race_simple=="White"][1]`%), followed by Asian (`r race_tab$pct[race_tab$URM_bin=="Non-URM" & race_tab$race_simple=="Asian"][1]`%). Among URM students, White (`r race_tab$pct[race_tab$URM_bin=="URM" & race_tab$race_simple=="White"][1]`%) and Asian (`r race_tab$pct[race_tab$URM_bin=="URM" & race_tab$race_simple=="Asian"][1]`%) students were less common, with higher representation in Black/African American (`r race_tab$pct[race_tab$URM_bin=="URM" & race_tab$race_simple=="Black / African American"][1]`%) and American Indian/Alaska Native (`r race_tab$pct[race_tab$URM_bin=="URM" & race_tab$race_simple=="American Indian / Alaska Native"][1]`%) categories. Missing race information was more common among URM students (`r race_tab$pct[race_tab$URM_bin=="URM" & is.na(race_tab$race_simple)][1]`%) compared with non-URM students (`r race_tab$pct[race_tab$URM_bin=="Non-URM" & is.na(race_tab$race_simple)][1]`%).
...
In the OSCE communication domain, the URM coefficient was essentially zero (estimate `r reg_osce$Estimate[reg_osce$Domain == "Communication" & reg_osce$Predictor == "URM (vs Non-URM)"]`; p = `r reg_osce[reg_osce$Domain == "Communication" & reg_osce$Predictor == "URM (vs Non-URM)", "p-value"]`), 
indicating no meaningful adjusted difference between groups. For Documentation, URM students scored an estimated 
`r reg_osce$Estimate[reg_osce$Domain == "Documentation" & reg_osce$Predictor == "URM (vs Non-URM)"]` points lower on average, but this difference was not statistically significant (p = `r reg_osce[reg_osce$Domain == "Documentation" & reg_osce$Predictor == "URM (vs Non-URM)", "p-value"]`). Similarly, in the Physical Exam domain, the adjusted URM difference was less than 1 point and non-significant (p = `r reg_osce[reg_osce$Domain == "Physical Exam" & reg_osce$Predictor == "URM (vs Non-URM)", "p-value"]`). Across all domains, gender and USMLE Step 2 CK scores showed consistent associations with OSCE performance, whereas USMLE Step 1 did not meaningfully contribute. Overall, these adjusted models provide no evidence that URM status independently predicts OSCE domain scores.
...
After adjusting for gender, URM students continued to score significantly lower on both Step examinations. URM students scored an estimated `r urm_step[urm_step$Outcome == "USMLE Step 1", "Adjusted Difference (URM – Non-URM)"]` points lower on Step 1 (95% CI: `r urm_step[urm_step$Outcome == "USMLE Step 1", "95% CI"]`; p = `r urm_step[urm_step$Outcome == "USMLE Step 1", "p-value"]`). For Step 2 CK, URM students scored `r urm_step[urm_step$Outcome == "USMLE Step 2 CK", "Adjusted Difference (URM – Non-URM)"]` points lower (95% CI: `r urm_step[urm_step$Outcome == "USMLE Step 2 CK", "95% CI"]`; p = `r urm_step[urm_step$Outcome == "USMLE Step 2 CK", "p-value"]`). These adjusted differences remained statistically significant and suggest that URM students experience notable performance gaps on standardized licensing examinations that are not explained by gender alone.
...
All OSCE domains showed positive correlations with USMLE Step performance. Associations were consistently stronger for Step 2 CK than for Step 1, with Step 2 CK correlations falling in the modest but statistically significant range. For USMLE Step 1 score, OSCE Communication and Documentation showed weak, non-significant correlations (r = `r cor_osce_usmle[cor_osce_usmle[["OSCE Domain"]] == "OSCE Communication" & cor_osce_usmle[["USMLE Exam"]] == "USMLE Step 1", "r"][[1]]` and r = `r cor_osce_usmle[cor_osce_usmle[["OSCE Domain"]] == "OSCE Documentation" & cor_osce_usmle[["USMLE Exam"]] == "USMLE Step 1", "r"][[1]]`, respectively), whereas Physical Exam showed a small but statistically significant association (r = `r cor_osce_usmle[cor_osce_usmle[["OSCE Domain"]] == "OSCE Physical Exam" & cor_osce_usmle[["USMLE Exam"]] == "USMLE Step 1", "r"][[1]]`; p = `r cor_osce_usmle[cor_osce_usmle[["OSCE Domain"]] == "OSCE Physical Exam" & cor_osce_usmle[["USMLE Exam"]] == "USMLE Step 1", "p-value"][[1]]`). For USMLE Step 2 CK score, all three OSCE domains showed statistically significant correlations, ranging from r = `r min(cor_osce_usmle[cor_osce_usmle[["USMLE Exam"]] == "USMLE Step 2 CK", "r"][["r"]])` to r = `r max(cor_osce_usmle[cor_osce_usmle[["USMLE Exam"]] == "USMLE Step 2 CK", "r"][["r"]])`, indicating modest but meaningful associations. Overall, these findings suggest that OSCE performance is more closely aligned with the applied clinical knowledge assessed by Step 2 CK than with the foundational science content assessed in Step 1.
...
Across all models, higher OSCE domain scores were associated with higher USMLE Step scores. For USMLE Step 1, only the OSCE Physical Exam domain showed a statistically significant positive association ($\beta$ = `r reg_table[reg_table$Outcome=="USMLE Step 1" & reg_table$Predictor=="OSCE Physical Exam", "Estimate"]`, 95% CI `r reg_table[reg_table$Outcome=="USMLE Step 1" & reg_table$Predictor=="OSCE Physical Exam", "CI"]`; p = `r reg_table[reg_table$Outcome=="USMLE Step 1" & reg_table$Predictor=="OSCE Physical Exam", "p-value"]`). OSCE Communication and Documentation domain scores showed positive but non-significant associations with Step 1 ($\beta$ = `r reg_table[reg_table$Outcome=="USMLE Step 1" & reg_table$Predictor=="OSCE Communication", "Estimate"]` and $\beta$ = `r reg_table[reg_table$Outcome=="USMLE Step 1" & reg_table$Predictor=="OSCE Documentation", "Estimate"]`, respectively).

For USMLE Step 2 CK, all three OSCE domains were significantly associated with exam performance. The strongest relationship was observed for Physical Exam ($\beta$ = `r reg_table[reg_table$Outcome=="USMLE Step 2 CK" & reg_table$Predictor=="OSCE Physical Exam", "Estimate"]`, 95% CI `r reg_table[reg_table$Outcome=="USMLE Step 2 CK" & reg_table$Predictor=="OSCE Physical Exam", "CI"]`; p = `r reg_table[reg_table$Outcome=="USMLE Step 2 CK" & reg_table$Predictor=="OSCE Physical Exam", "p-value"]`). Communication ($\beta$ = `r reg_table[reg_table$Outcome=="USMLE Step 2 CK" & reg_table$Predictor=="OSCE Communication", "Estimate"]`) and Documentation ($\beta$ = `r reg_table[reg_table$Outcome=="USMLE Step 2 CK" & reg_table$Predictor=="OSCE Documentation", "Estimate"]`) also showed statistically significant associations.
